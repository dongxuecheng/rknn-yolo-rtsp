...
int inference_yolov8_pose_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results, float conf_threshold, float nms_threshold)
{
    int ret;
    ...
    float nms_thresh = (nms_threshold > 0.0f) ? nms_threshold : YOLOV8_POSE_NMS_THRESH;
    float box_conf_thresh = (conf_threshold > 0.0f) ? conf_threshold : YOLOV8_POSE_BOX_THRESH;
    int bg_color = 114;
    ...
    // Post Process
    yolov8_pose_post_process(app_ctx, outputs, &letter_box, box_conf_thresh, nms_thresh, od_results);
